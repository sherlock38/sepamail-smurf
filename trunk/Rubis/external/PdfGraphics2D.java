...
public class PdfGraphics2D extends Graphics2D {
    ...
    private BaseFont normalFont = null; // Added by Bishan
    private BaseFont boldFont = null;   // Added by Bishan
    ...
    // Set the normal font to be used
    public void setNormalFont(BaseFont normalFont) {
        this.normalFont = normalFont;
    }

    // Set the bold font to be used
    public void setBoldFont(BaseFont boldFont) {
...
//            drawGlyphVector(this.font.createGlyphVector(getFontRenderContext(), s), x, y);
            ...
            // Added by Bishan
            String fontFullName = "";
            for (int i = 0; i < baseFont.getFullFontName().length; i++) {
                for (int j = 0; j < baseFont.getFullFontName()[i].length; j++) {
                    fontFullName += baseFont.getFullFontName()[i][j];
                }
                fontFullName += " ";
            }

            // Added by Bishan
            fontFullName = fontFullName.trim();

            // Added by Bishan
            if ((fontFullName.indexOf("Bold") > -1) || (fontFullName.indexOf("bold") > -1)) {
                if (this.boldFont == null) {
                    cb.setFontAndSize(baseFont, fontSize);
                } else {
                    cb.setFontAndSize(this.boldFont, fontSize);
                }
            } else {
                if (this.normalFont == null) {
                    cb.setFontAndSize(baseFont, fontSize);
                } else {
                    cb.setFontAndSize(this.normalFont, fontSize);
                }
            }
